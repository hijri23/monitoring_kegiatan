	...
	$(document).ready(function() {
		...
		Morris.Area({
			...
			data: [
				...
				$arsip = mysqli_query($koneksi, "SELECT * FROM arsip WHERE date(arsip_waktu_upload) >= '$awal' AND date(arsip_waktu_upload) <= '$akhir' AND arsip_petugas = $petugas_id");
				while ($p = mysqli_fetch_array($arsip)) {
					$tgl = date('Y/m/d', strtotime($p['arsip_waktu_upload']));
					$jumlah = mysqli_query($koneksi, "select * from arsip where date(arsip_waktu_upload)='$tgl' AND arsip_petugas = $petugas_id");
